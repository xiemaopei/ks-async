...
public:
	...
	virtual bool __do_run_nested_pump_loop_for_extern_waiting(void* object, std::function<bool()>&& extern_pred_fn) { ASSERT(false); throw std::runtime_error("this apartment doesn't support nested pump-loop"); }
	virtual void __do_notify_nested_pump_loop_for_extern_waiting(void* object) { ASSERT(false); throw std::runtime_error("this apartment doesn't support nested pump-loop"); }
