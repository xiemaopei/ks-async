...
protected:
	...
	void do_init_base_locked(ks_apartment* spec_apartment, const ks_async_context& living_context, const std::shared_ptr<__INTERMEDIATE_DATA>& intermediate_data_ptr, std::unique_lock<ks_mutex>& lock) {
...
protected:
	...
	void do_complete_locked(const ks_raw_result& completed_result, ks_apartment* prefer_completed_apartment, bool from_internal, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		ASSERT(lock.owns_lock() && !must_keep_locked);
		...
		if (prefer_completed_apartment == nullptr)
			prefer_completed_apartment = this->do_determine_prefer_apartment(intermediate_data_ptr != nullptr ? intermediate_data_ptr->m_spec_apartment : nullptr, nullptr);

		m_completed_result = completed_result.require_completed_or_error();
		m_completed_prefer_apartment = prefer_completed_apartment;
		...
		if (intermediate_data_ptr != nullptr) {
			...
			__clear_intermediate_data_ptr(intermediate_data_ptr, lock); //completed后清除
...
protected:
#if __KS_ASYNC_RAW_FUTURE_MUTEX_GLOBAL_SINGLETON_ENABLED
	static ks_mutex m_mutex;
#else
	ks_mutex m_mutex;
#endif
	...
	struct __INTERMEDIATE_DATA {
		...
		ks_raw_future_ptr m_next_future_1st;
...
#if __KS_ASYNC_RAW_FUTURE_MUTEX_GLOBAL_SINGLETON_ENABLED
ks_mutex ks_raw_future_baseimp::m_mutex;
#endif
...
public:
	...
	void init(ks_apartment* spec_apartment) {
		std::unique_lock<ks_mutex> lock(m_mutex);
		do_init_base_locked(spec_apartment, ks_async_context::__empty_inst(), m_intermediate_data_ex_ptr, lock);
	}
...
private:
	...
	virtual void __clear_intermediate_data_ptr(const std::shared_ptr<__INTERMEDIATE_DATA>& intermediate_data_ptr, std::unique_lock<ks_mutex>& lock) override {
...
private:
	...
	void do_submit_locked(std::function<ks_raw_result()>&& task_fn, int64_t delay, const std::shared_ptr<__INTERMEDIATE_DATA_EX>& intermediate_data_ex_ptr, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		ASSERT(intermediate_data_ex_ptr != nullptr);
		...
		m_intermediate_data_ex_ptr->m_delay = delay;

		ks_apartment* prefer_apartment = this->do_determine_prefer_apartment(intermediate_data_ex_ptr->m_spec_apartment, nullptr);
...
private:
	struct __INTERMEDIATE_DATA_EX;
	void do_connect_locked(std::function<ks_raw_result(const ks_raw_result&)>&& fn_ex, const ks_raw_future_ptr& prev_future, const std::shared_ptr<__INTERMEDIATE_DATA_EX>& intermediate_data_ex_ptr, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		ASSERT(intermediate_data_ex_ptr != nullptr);
		ASSERT(intermediate_data_ex_ptr == __get_intermediate_data_ex_ptr(lock));
		ASSERT(lock.owns_lock() && !must_keep_locked);
		ASSERT(!m_completed_result.is_completed());
...
protected:
	virtual void on_feeded_by_prev(const ks_raw_result& prev_result, ks_raw_future* prev_future, ks_apartment* prev_advice_apartment) override {
		...
		if (could_skip_run) {
			...
			ks_apartment* prefer_apartment = this->do_determine_prefer_apartment(intermediate_data_ex_ptr->m_spec_apartment, prev_advice_apartment);
			this->do_complete_locked(prev_result, prefer_apartment, true, lock, false);
			return;
		}

		if (true && this->do_check_cancel_locking(lock)) {
		...
		std::function<void()> run_fn = [this, this_shared = this->shared_from_this(), intermediate_data_ex_ptr, prev_result, prefer_apartment, context = intermediate_data_ex_ptr->m_living_context]() mutable -> void {
			...
			ks_raw_result result;
			try {
				if (true && this->do_check_cancel_locking(lock2))
...
private:
	struct __INTERMEDIATE_DATA_EX;
	void do_connect_locked(std::function<ks_raw_future_ptr(const ks_raw_result&)>&& afn_ex, const ks_raw_future_ptr& prev_future, const std::shared_ptr<__INTERMEDIATE_DATA_EX>& intermediate_data_ex_ptr, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		ASSERT(intermediate_data_ex_ptr != nullptr);
		ASSERT(intermediate_data_ex_ptr == __get_intermediate_data_ex_ptr(lock));
		ASSERT(lock.owns_lock() && !must_keep_locked);
		ASSERT(!m_completed_result.is_completed());
		...
		}, context, intermediate_data_ex_ptr->m_spec_apartment);
...
private:
	struct __INTERMEDIATE_DATA_EX;
	void do_connect_locked(const std::vector<ks_raw_future_ptr>& prev_futures, const std::shared_ptr<__INTERMEDIATE_DATA_EX>& intermediate_data_ex_ptr, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		ASSERT(intermediate_data_ex_ptr != nullptr);
		ASSERT(intermediate_data_ex_ptr == __get_intermediate_data_ex_ptr(lock));
		ASSERT(lock.owns_lock() && !must_keep_locked);
		ASSERT(!m_completed_result.is_completed());
	...
	void do_check_and_try_settle_me_locked(const ks_raw_result& prev_result, ks_apartment* prev_advice_apartment, std::unique_lock<ks_mutex>& lock, bool must_keep_locked) {
		...
		case ks_raw_future_mode::ALL:
			if (intermediate_data_ex_ptr->m_prev_first_rejected_index != -1) {
				//前序任务出现失败
				ks_apartment* prefer_apartment = do_determine_prefer_apartment(intermediate_data_ex_ptr->m_spec_apartment, prev_advice_apartment);
